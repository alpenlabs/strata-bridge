...
/// Path to the `Sp1Groth16:<hex>` predicate string emitted next to the
/// counterproof ELF by `build.rs`.
pub const BRIDGE_COUNTERPROOF_PREDICATE_PATH: &str =
    concat!(env!("CARGO_MANIFEST_DIR"), "/elfs/counterproof.predicate");
