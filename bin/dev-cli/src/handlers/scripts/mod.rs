...
use bitcoin::{secp256k1::XOnlyPublicKey, Amount, TapNodeHash};

pub(in crate::handlers) mod general;

/// Metadata bytes that the Bridge uses to read information from the bitcoin blockchain and the
/// sidesystem.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::handlers) struct AuxiliaryData {
    /// Tag, also known as "magic bytes".
    pub(in crate::handlers) tag: Tag,

    /// Deposit-specific metadata.
    pub(in crate::handlers) metadata: DepositMetadata,
}

/// Deposit-specific metadata that the Bridge uses to read information from the bitcoin blockchain
/// and the sidesystem.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::handlers) enum DepositMetadata {
...
impl AuxiliaryData {
    ...
    pub(in crate::handlers) const fn new(tag: Tag, metadata: DepositMetadata) -> Self {
        Self { tag, metadata }
    }

    /// Extracts the metadata as bytes.
    pub(in crate::handlers) fn to_vec(&self) -> Vec<u8> {
