...
pub(in crate::handlers) fn n_of_n_script(aggregated_pubkey: &XOnlyPublicKey) -> Script {
    script! {
        { *aggregated_pubkey }
        OP_CHECKSIG
    }
...
pub(in crate::handlers) fn drt_take_back(
...
pub(in crate::handlers) fn n_of_n_with_timelock(
...
pub(in crate::handlers) fn op_return_nonce(data: &[u8]) -> ScriptBuf {
...
pub(in crate::handlers) fn get_aggregated_pubkey(
...
pub(in crate::handlers) fn metadata_script(auxiliary_data: AuxiliaryData) -> ScriptBuf {
...
pub(in crate::handlers) fn anyone_can_spend_script() -> ScriptBuf {
    script! {
        OP_TRUE
    }
    .compile()
...
pub(in crate::handlers) fn anyone_can_spend_txout() -> TxOut {
...
pub(in crate::handlers) const fn create_tx(tx_ins: Vec<TxIn>, tx_outs: Vec<TxOut>) -> Transaction {
    Transaction {
...
pub(in crate::handlers) fn create_tx_ins(utxos: impl IntoIterator<Item = OutPoint>) -> Vec<TxIn> {
...
pub(in crate::handlers) fn create_tx_outs(
