...
///
/// This should have its own unique key that isn't used for any other purpose.
...
pub struct SignerIdxOutOfBounds {
    /// Index we were trying to access.
    pub index: usize,
    /// Number of signers in the session.
    pub n_signers: usize,
}

/// The musig2 signer trait is used to bootstrap and begin a musig2 session.
/// A single secret key should be used across all sessions initiated by this signer,
/// whose public key should be accessible via the `pubkey` method.
pub trait Musig2Signer<O: Origin, FirstRound>: Send + Sync {
    /// Initialize a new musig2 session with the given public keys, witness, input transaction ID,
    /// and input vout. `pubkeys` may or may not include our own pubkey and should be added if not
    /// included by implementor. `pubkeys` may or may not be sorted, so should be sorted
...
/// by the `new_session` method of the `Musig2Signer` trait.
///
...
/// implementors should provide their own implementation server-side.
pub trait Musig2SignerFirstRound<O: Origin, SecondRound>: Send + Sync {
    /// Returns our public nonce which should be shared with other signers.
    ...
    /// index. Returns an error if the signer index is out of range, or if we already have a
...
pub trait Musig2SignerSecondRound<O: Origin>: Send + Sync {
    ...
    /// the set of signers, or if we already have a different partial signature on-file for that
    /// signer.
...
/// Winternitz One-Time Signatures are used to transfer state across UTXOs, even though
/// bitcoin does not support this natively. This signer returns deterministic keys so the
/// caller can assemble a transaction.
...
/// The stakechain preimages struct is used to generate deterministic preimages for the stakechain
/// used for withdrawals.
pub trait StakeChainPreimages<O: Origin>: Send {
    /// Returns a deterministic preimage for a given stakechain withdrawal through a given txid,
    /// vout and stake index.
    fn get_preimg(
        &self,
        prestake_txid: Txid,
        prestake_vout: u32,
...
/// The origin trait is used to parameterize the main secret service traits so
/// that clients and servers alike can implement a single trait, but clients
/// will receive the server's response wrapped in a result with other spurious
/// network or protocol errors it may encounter.
pub trait Origin {
...
/// Enforcer for other traits to ensure implementations only work for the client & provides
/// container type
#[derive(Debug)]
pub struct Client;
impl Origin for Client {
    // for the client, we wrap responses in a result that may have a client error
...
pub enum ClientError {
    ...
    /// We failed to deserialize something. Server is giving us bad responses
    BadData,
    /// We failed to write data towards the server
    WriteError(WriteError),
    /// We failed to read data from the server
