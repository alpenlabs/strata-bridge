...
pub fn not<A>(f: impl Fn(&A) -> bool) -> impl Fn(&A) -> bool {
    move |a| !f(a)
}

/// Predicate combinator for the && operation.
pub fn and<A>(f: impl Fn(&A) -> bool, g: impl Fn(&A) -> bool) -> impl Fn(&A) -> bool {
    move |a| f(a) && g(a)
}
...
pub fn nand<A>(f: impl Fn(&A) -> bool, g: impl Fn(&A) -> bool) -> impl Fn(&A) -> bool {
    move |a| boolean::nand(f(a), g(a))
}

/// Predicate combinator for the nor operation.
pub fn nor<A>(f: impl Fn(&A) -> bool, g: impl Fn(&A) -> bool) -> impl Fn(&A) -> bool {
    move |a| boolean::nor(f(a), g(a))
}

/// Contravariant functor map over predicates.
pub fn contramap<A, B>(f: impl Fn(&A) -> B, p: impl Fn(&B) -> bool) -> impl Fn(&A) -> bool {
    move |a| p(&f(a))
}
