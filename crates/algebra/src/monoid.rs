...
/// [`concat`] is the free catamorphism (fold) over a Monoidal iterator. If the iterator has no
/// elements, it returns [`Monoid::empty`], otherwise it folds the iterator using
/// [`Semigroup::merge`].
pub fn concat<T: Monoid>(xs: impl IntoIterator<Item = T>) -> T {
    xs.into_iter().fold(T::empty(), <T as Semigroup>::merge)
}

/// [`fold_map`] is the universal catamorphism over all iterators. If given the iterant type has a
/// morphism into a Monoidal structure this function will use that Monoidal structure to fold.
