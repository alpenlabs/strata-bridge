...
use crate::{errors::DecodeError, rkyv_wrappers, tagged_hashes, BinaryEncoding};
...
#[derive(Debug, Eq, PartialEq, Clone, Archive, Serialize, Deserialize)]
pub struct SecNonce {
    #[rkyv(with = rkyv_wrappers::Scalar)]
    pub(crate) k1: Scalar,
    #[rkyv(with = rkyv_wrappers::Scalar)]
    pub(crate) k2: Scalar,
}
...
impl SecNonce {
    ...
    ///
    /// - `nonce_seed`: the primary source of entropy used to generate the nonce. Can be any type
    ///   that converts to [`NonceSeed`], such as [`&mut rand::rngs::OsRng`][rand::rngs::OsRng] or
    ///   `[u8; 32]`.
    ...
    /// - `extra_input`: arbitrary context data used to increase the entropy of the resulting
    ///   nonces.
    ///
...
#[derive(Debug, Eq, PartialEq, Clone, Hash, Ord, PartialOrd, Archive, Serialize, Deserialize)]
pub struct PubNonce {
    #[allow(missing_docs)]
...
impl AggNonce {
    ...
    /// ];
    ///
    /// let expected = "02aebee092fe428c3b4c53993c3f80eecbf88ca935469b5bfcaabecb7b2afbb1a6\
