...
use crate::{
    errors::{DecodeError, InvalidSecretKeysError, KeyAggError, TweakError},
    rkyv_wrappers, tagged_hashes, BinaryEncoding,
};
...
impl BinaryEncoding for KeyAggContext {
    ...
    ///
    /// - `header_byte` (1 byte)
    ///     - Lowest order bit is set if the parity of the aggregated pubkey should be negated upon
    ///       deserialization (due to use of "x-only" tweaks).
    ///     - Second lowest order bit is set if there is an accumulated tweak value present in the
    ///       serialization.
    ...
    ///     - The public keys needed to reconstruct the `KeyAggContext`, in the same order in which
