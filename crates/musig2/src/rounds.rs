...
use crate::{
    errors::{RoundContributionError, RoundFinalizeError, SignerIndexError, SigningError},
    rkyv_wrappers::{self},
...
#[derive(Archive, Serialize, Deserialize)]
#[rkyv(remote = Slots<PartialSignature>)]
struct PartialSignatureSlots {
    #[rkyv(with = Map<Map<rkyv_wrappers::MaybeScalar>>)]
    slots: Vec<Option<PartialSignature>>,
    open_slots: Vec<usize>,
}

impl From<PartialSignatureSlots> for Slots<PartialSignature> {
    fn from(partial_signature_slots: PartialSignatureSlots) -> Self {
        let PartialSignatureSlots { slots, open_slots } = partial_signature_slots;
        Slots { slots, open_slots }
    }
}

impl From<Slots<PartialSignature>> for PartialSignatureSlots {
    fn from(partial_signature_slots: Slots<PartialSignature>) -> Self {
        let Slots { slots, open_slots } = partial_signature_slots;
        PartialSignatureSlots { slots, open_slots }
    }
}
...
impl FirstRound {
    ...
    /// - Once you receive the aggregated nonce, use [`FirstRound::sign_for_aggregator`] instead of
    ///   [`finalize`][Self::finalize] to consume the `FirstRound` and return a partial signature.
    ...
    ///   which can be verified with [`verify_single`][crate::verify_single]
    ///
    ...
    ///   instead of [`finalize_adaptor`][Self::finalize_adaptor] to consume the `FirstRound` and
...
pub struct SecondRound<M: AsRef<[u8]>> {
    ...
    aggnonce: AggNonce,
    #[rkyv(with = rkyv_wrappers::MaybePoint)]
    adaptor_point: MaybePoint,
    message: M,
    #[rkyv(with = PartialSignatureSlots)]
