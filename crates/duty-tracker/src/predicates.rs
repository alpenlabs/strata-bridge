...
pub(crate) fn parse_strata_checkpoint(
    ...
    // Try verifying against the rollup params credential rule first
    if verify_signed_checkpoint_sig(&signed_checkpoint, &rollup_params.cred_rule) {
        return Some(signed_checkpoint.into());
    }

    // Try verifying against the alternative credential rule if provided
    if let Some(cred) = alternative_cred {
        if verify_signed_checkpoint_sig(&signed_checkpoint, cred) {
            return Some(signed_checkpoint.into());
        }
    }
