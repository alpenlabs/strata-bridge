...
pub static PARTIAL_VERIFIER_SCRIPTS: LazyLock<[Script; N_TAPLEAVES]> =
...
pub fn load_or_create_verifier_scripts() -> [Script; N_TAPLEAVES] {
    if cfg!(feature = "mock") {
        warn!("Detected mock feature, returning empty verifier scripts");

        return vec![script!(); N_TAPLEAVES]
            .try_into()
            .expect("size must match");
    }

    let verifier_scripts: [Script; N_TAPLEAVES] = if fs::exists(PARTIAL_VERIFIER_SCRIPTS_PATH)
...
pub fn get_verifier_scripts() -> &'static [Script; N_TAPLEAVES] {
