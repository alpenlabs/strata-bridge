...
use crate::{
    graph::{
        ...
        tests::{
            CLAIM_BLOCK_HEIGHT, CONTEST_TIMELOCK_BLOCKS, GraphInvalidTransition, GraphTransition,
            INITIAL_BLOCK_HEIGHT, create_sm, get_state, mock_game_signatures,
...
fn test_new_block_claimed_timeout_triggers_payout() {
    ...
    let uncontested_sigs = GameFunctor::unpack(signatures.clone(), cfg.watchtower_pubkeys.len())
    ...
    test_transition::<GraphSM, _, _, _, _, _, _, _>(
        ...
        cfg,
        GraphTransition {
            from_state: claimed_state(INITIAL_BLOCK_HEIGHT, fulfillment_txid, signatures.clone()),
            event: GraphEvent::NewBlock(NewBlockEvent {
                block_height: new_height,
            }),
