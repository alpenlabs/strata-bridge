...
impl BtcZmqSM {
    ...
    pub(crate) fn process_block(&mut self, block: Block) -> Vec<TxEvent> {
        ...
        match self.unburied_blocks.front() {
            Some(tip) => {
                if block.header.prev_blockhash == tip.block_hash() {
                    ...
                    // be fixed.
                    error!(block_hash=%block.block_hash(), prev_block_hash=%tip.block_hash(), "Block's previous block hash does not match the tip");
        ...
        trace!(?block, "Started processing transactions in the block");
        ...
        {
            trace!(?matched_tx, "Started processing transaction");
            match self.tx_lifecycles.get_mut(&matched_tx.compute_txid()) {
                ...
                None | Some(None) => {
                    ...
                    debug!(txid=%matched_tx.compute_txid(), blockhash=%blockhash, %height, "Processing new transaction");
                    ...
                    info!(txid=%matched_tx.compute_txid(), blockhash=%blockhash, %height, "Processed new transaction");
                }
                // This means we have seen the rawtx event for this transaction before.
                Some(Some(lifecycle)) => {
                    let blockhash = block.block_hash();
                    let height = block.bip34_block_height().unwrap_or(0);
                    debug!(txid=%matched_tx.compute_txid(), blockhash=%blockhash, %height, "Processing already seen transaction");
                    if let Some((_, prior_blockhash)) = lifecycle.block {
                        ...
                        warn!(txid=%matched_tx.compute_txid(), blockhash=%blockhash, %height, "Transaction already mined");
        ...
        if self.unburied_blocks.len() > self.bury_depth {
            if let Some(newly_buried) = self.unburied_blocks.pop_back() {
                ...
                trace!(?newly_buried, %blockhash, %height, "Handled all mined transactions. Starting to process buried transactions from block");
                info!(%blockhash, %height, "Handled all mined transactions. Starting to process buried transactions from block");
                for buried_tx in newly_buried.txdata {
                    let buried_txid = buried_tx.compute_txid();
                    trace!(?buried_tx, %buried_txid, %blockhash, %height, "Handled all mined transactions. Starting to process specific buried transactions");
                    debug!(%buried_txid, %blockhash, %height, "Handled all mined transactions. Starting to process specific buried transactions");
                    ...
                    info!(%blockhash, %height, "Handled all mined and buried transaction from block");
                }
            }
        }
    ...
    pub(crate) fn process_tx(&mut self, tx: Transaction) -> Vec<TxEvent> {
        ...
        trace!(?tx, %txid, "Processing transaction");
        info!(%txid, "Processing transaction");
        ...
        match lifecycle {
            ...
            None => {
                trace!(?tx, %txid, ?lifecycle, "Never seen transaction");
                debug!(%txid, ?lifecycle, "Never seen transaction");
                let lifecycle = TxLifecycle {
                    raw: tx,
                    block: None,
                };
            ...
            Some(None) => {
                trace!(?tx, %txid, ?lifecycle, "Have seen a MempoolAcceptance event for this transaction");
                debug!(%txid, ?lifecycle, "Have seen a MempoolAcceptance event for this transaction");
                let lifecycle = TxLifecycle {
                    raw: tx.clone(),
                    block: None,
                };
            ...
            Some(Some(_)) => {
                trace!(?tx, %txid, ?lifecycle, "Already know everything that needs to be known about this transaction");
                debug!(%txid, ?lifecycle, "Already know everything that needs to be known about this transaction");
                Vec::new()
            }
        }
    }
            ...
            SequenceMessage::BlockDisconnect { blockhash } => {
                ...
                if let Some(block) = self.unburied_blocks.front() {
                    if block.block_hash() == blockhash {
                        info!(%blockhash, "Block disconnected, removing all transactions");
                ...
                self.tx_lifecycles.retain(|_, v| {
                    match v {
                        Some(lifecycle) => match lifecycle.block {
                            ...
                            // Otherwise keep it.
                            _ => {
                                trace!(tx=?lifecycle.raw, txid=%lifecycle.raw.compute_txid(), "Block disconnected, keeping transaction");
                                true
                            }
                        },
                        None => {
                            trace!(?v, "Block disconnected, keeping transaction");
                            true
                        },
                    }
                });
            ...
            SequenceMessage::MempoolAcceptance { txid, .. } => {
                ...
                match self.tx_lifecycles.get_mut(&txid) {
                    ...
                    Some(Some(lifecycle)) => {
                        match lifecycle.block {
                            ...
                            None => {
                                ...
                                trace!(?diff, ?seq, %txid, "MempoolAcceptance received before rawtx");
                            }
                            ...
                            Some(_) => {
                                trace!(?diff, ?seq, %txid, "MempoolAcceptance received but probably a race condition between rawblock and sequence events");
                                /* NOOP */
                            }
                        }
                    }
                    ...
                    None => {
                        ...
                        trace!(?diff, ?seq, %txid, "MempoolAcceptance received and don't have the rawtx yet");
            ...
            SequenceMessage::MempoolRemoval { txid, .. } => {
                match self.tx_lifecycles.remove(&txid) {
                    ...
                    Some(Some(lifecycle)) => {
                        ...
                        trace!(?diff, ?seq, %txid, "MempoolRemoval received for a transaction that has been seen as rawtx before");
                    }
                    // This will happen if we've only received a MempoolAcceptance event, the
                    // removal will cancel it fully.
                    Some(None) => {
                        trace!(?diff, ?seq, %txid, "MempoolRemoval received for a transaction that has been seen as MempoolAcceptance before");
                        /* NOOP */
                    }
                    // This happens if we've never heard anything about this transaction before.
                    None => {
                        trace!(?diff, ?seq, %txid, "MempoolRemoval received for a transaction that no information is available yet");
                        /* NOOP */
                    }
                }
            }
